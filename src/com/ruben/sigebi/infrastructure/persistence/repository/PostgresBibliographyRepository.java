...
public class PostgresBibliographyRepository implements BibliographyRepository {
    ...
    @Override
    public void save(NormalPhysicalBibliographyResource resource) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            stmt.setObject(1, resource.getBaseResourceID().value());
    ...
    @Override
    public Optional<NormalPhysicalBibliographyResource> findById(ResourceID id) {
    ...
    @Override
    public List<NormalPhysicalBibliographyResource> findByAuthorId(AuthorId authorId) {
        String sql = "SELECT * FROM bibliography_resources WHERE author_id = ?";
        List<NormalPhysicalBibliographyResource> list = new ArrayList<>();
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
