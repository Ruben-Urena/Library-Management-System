...
public class Loan extends ActivatableAggregate {
    ...
    private final Set<ReservationCode> reservationCodes;
    private final Set<ReturnCode> returnCodes;
    ...
    public Loan(UserId userId, UUID loanId, ResourceCopyId copyId, Instant endDate) {
        ...
        reservationCodes = new HashSet<>();
        returnCodes = new HashSet<>();
        activate();
    }
    ...
    public void addReservationCode(ReservationCode reservationCode){
        Objects.requireNonNull(reservationCode, "reservation code cannot be null");
        this.reservationCodes.add(reservationCode);
    }
    public void addReturnCode(ReturnCode returnCode) {
        Objects.requireNonNull(returnCode, "return code cannot be null");
        this.returnCodes.add(returnCode);
    }
    public void addAllReservationCode(Set<ReservationCode> reservationCodes){
        this.reservationCodes.addAll(reservationCodes);
    }
    public void addAllReturnCode(Set<ReturnCode> returnCodes){
        this.returnCodes.addAll(returnCodes);
    }

    public Set<ReservationCode> getReservationCodes() {
        return Set.copyOf(reservationCodes);
    }

    public Set<ReturnCode> getReturnCodes() {
        return Set.copyOf(returnCodes);
    }
